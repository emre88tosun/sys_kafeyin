                                ...
                                @if(count($user->brand->notifs->concat($user->magaza->notifs)) > 0)
                                    <div class="slimscroll noti-scroll2" style="min-height: 650px">
                                        @foreach($user->brand->notifs->concat($user->magaza->notifs)->sortByDesc('created_at') as $notif)
                                            <div class="notify-item border-bottom @if(!$notif->isSeen) bg-soft-primary rounded-lg @endif " style="padding: 12px">
                                                <div class="notify-icon">
                                                    <img src="{{ URL::asset('assets/images/white-bg-only-cup.png') }}" height="36" width="36" class="img-fluid rounded-circle" alt="" />
                                                </div>
