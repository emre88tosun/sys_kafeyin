    ...
    <div class="col-md-12">
        <nav aria-label="breadcrumb" class="float-right mt-1">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Kullanıcı Anketleri</li>
            </ol>
        </nav>
        <h4 class="mb-1 mt-0">Kullanıcı Anketleri</h4>
    </div>
</div>
@endsection
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                @if(count($anket->answers->where('additionalMessage','!=',null)))
                                                    <a href="javascript:void(0);" type="button" data-toggle="modal"
                                                       data-target="#ekstraModal{{$anket->id}}"
                                                       class="dropdown-item">Ekstra Mesajlar</a>
                                                @endif
                                                @if($anket->isActive)
                                                    ...
                                                    <a href="javascript:void(0);"
                                                       type="button"
                                                       data-id="{{$anket->id}}"
                                                       class="dropdown-item sa-kAnketPasif">Pasifize
                                                        et</a>
                                                @endif
                                                <a href="javascript:void(0);" type="button" data-id="{{$anket->id}}"
                                                   class="dropdown-item text-danger sa-kAnketSil">Sil</a>
                        ...
                        @foreach($ankets as $anket)
                            ...
                            <div class="modal fade" id="duzenleModal{{$anket->id}}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="post" action="/adminpanel/kanketguncelle">
                                                @csrf
                                                <fieldset>
                            ...
                            <div class="modal fade" id="ekstraModal{{$anket->id}}" tabindex="-1" role="dialog"
                                ...
                                <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="ekstraModal{{$anket->id}}">Ekstra mesajlar</h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Kapat">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body">
                                            ...
                                            <p>{{$anket->desc}}</p>
                                            <hr>
                                            @foreach($anket->answers->where('additionalMessage','!=',null) as $answer)
                                                <p>{{$answer->additionalMessage}}</p>
                                                <a href="/adminpanel/kullanicilar/normal/{{$answer->user->id}}" class="text-muted text-dark">#{{$answer->user->id}} - {{$answer->user->name}} {{$answer->user->surname}}</a>
                                                <hr>
                                            @endforeach
                                        </div>
                                    </div>
        ...
        <div class="col-xl-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="post" action="/adminpanel/kanketekle">
                        @csrf
                        <fieldset>
                            <div class="form-group row">
                                <label for="title"
